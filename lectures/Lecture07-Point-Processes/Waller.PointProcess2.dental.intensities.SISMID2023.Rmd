...
# Define the relative risk surface as the point-by-point ratio of case density to control density
...
#pdf(paste("dentalKD.tol700.KS.pdf",width=11,height=8)
    ...
    persp(casdens$x1,casdens$x2,r.ks,theta=-25,phi=45,xlab="u",ylab="v",zlab="log relative risk",zlim=c(-3,3))
...
hist(test.stat.sim,nclass=25,
         ...
         round(length(test.stat.sim[test.stat.sim>test.stat.obs])/length(test.stat.sim),4)))
...
Since this example doesn't have strong clustering, the only symbol added are 2 "-" symbols along the lower left edge.   
